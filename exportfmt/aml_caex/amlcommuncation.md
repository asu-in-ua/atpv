...
# AutomationML Communication конспект
...
Логічна топологія `logicalTopology`  описує логічне подання комунікаційних партнерів з точки зору прикладних програм. `logicalTopology` - агрегує всі комунікаційні відношення між логічними кінцевими точками, пов’язаними з пристроями автоматизації.

- включає `logicalConnection`  - комунікаційне відношення між двома логічними кінцевими точками, 
  - включає `logicalEndPoint` - представляє комунікаційного партнера в такому відношенні.
    - включає `logicalEndPoint`...

Фізична топологія `physicalTopology`  описує фізичне подання комунікаційних партнерів. Вона представляє комунікаційні відношення з точки зору проводки або налаштувань каналів. На рисунку 19 показано три задіяні (абстрактні) елементи та їх взаємозв’язки:

`physicalTopology`  - є точкою входу для моделювання фізичної топології мережі автоматизації. агрегує всі комунікаційні відношення між фізичними кінцевими точками, пов’язаними з пристроями автоматизації.

- `physicalConnection` - описує комунікаційне відношення між двома фізичними кінцевими точками. Властивість `physicalConnectionId` є GUID у межах установки та ідентифікує `physicalConnection`. Властивість `physicalEndPointIdRefs` є масивом, що містить усі GUID фізичних кінцевих точок (`physicalEndPoint`), пов’язаних із даним `physicalConnection`. Таким чином можуть бути змодельовані з’єднання типу m:n у комунікаційній мережі. Реальним похідним варіантом `physicalConnection` може бути кабель, що з’єднує роз’єми у відношенні типу peer-to-peer.
  - `physicalEndPoint` - представляє реальну кінцеву точку зв’язку в такому відношенні. Властивість `physicalEndPointId` є GUID у межах системи автоматизації та ідентифікує `physicalEndPoint`. Властивість `logicalEndPointIdRef` є GUID логічної кінцевої точки (`logicalEndPoint`), пов’язаної з даним `physicalEndPoint`. Похідним прикладом такої кінцевої точки комунікації може бути Ethernet-порт.
    - `physicalEndPoint` ...
...
# Whitepaper AutomationML Communication

Нижче переклад [Whitepaper AutomationML Communication Document Identifier: WP Comm, V 1.0.0 State: September 2014 ](https://www.automationml.org/download-archive/)
...
Крок 1. Проєктування структури технічних засобів системи (system instrumentation), визначення використовуваних та взаємопов’язаних пристроїв
Крок 2. Експорт інформації про пристрої з інструменту проєктування системи (system instrumentation tool)
Крок 3. Імпорт інформації про пристрої до інструменту програмування PLC та конфігурації пристроїв
Крок 4. Інтеграція описів пристроїв (наприклад, GSDML) в інструменті програмування PLC
Крок 5. Проєктування програм і конфігурацій PLC в інструменті програмування PLC
Крок 6. Експорт релевантної для комунікаційних пристроїв інформації з інструменти програмування PLC
Крок 7. Імпорт релевантної для комунікаційних пристроїв інформації до інструменти конфігурації пристроїв
...
Існують різні альтернативні варіанти, проте зазвичай вони не застосовуються. Нижче наведено послідовності, які є можливими та перебуватимуть у фокусі цього сценарію використання.

Крок 1. Проєктування конфігурацій комунікаційної системи в сторонньому інструменті
Крок 2. Інтеграція описів пристроїв
Крок 3. Експорт релевантної для комунікаційних пристроїв інформації
Крок 4. Імпорт релевантної для комунікаційних пристроїв інформації до інструменту конфігурації або програмування пристроїв
...
Крок 1. Стороння інтсрумента, наприклад інтсрумента конфігурації пристроїв, надає інформацію, таку як сигнали та обсяг даних, що описує інформацію рівня екземпляра
Крок 2. Виробник пристрою надає описи пристроїв, що описують інформацію рівня типу
Крок 3. Інструмент конфігурації шини споживає ці фрагменти даних та описи пристроїв
Крок 4. Конфігуратор шини генерує конфігурацію шини
Крок 5. Імпорт конфігурації шини до інструменти програмування PLC
Крок 6. Використання інформації з периферійних пристроїв у програмі PLC
...
Можливими інструментами, що експортують інформацію про систему зв’язку, можуть бути, наприклад, інструменти програмування PLC. Вони охоплюють проєкти програмування PLC з точками даних (змінними), конфігураціями пристроїв та непрямими описами комунікаційної мережі. Крім того, джерелами даних можуть бути інструменти інженерії комунікаційних систем, а також інструменти проєктування приладового та електричного оснащення, наприклад ECAD-інструменти.

Приймачами даних у процесі обміну можуть бути інструменти конфігурації зв’язку датчиків, програмування HMI, програмування роботів, конфігурації OPC-систем або конфігурації зв’язку виконавчих механізмів. Вони здебільшого також охоплюють проєкти програмування з точками даних (змінними), конфігураціями пристроїв та непрямими описами комунікаційної мережі. Релевантними можуть бути FDT-інструменти, інструменти програмування HMI та інструменти програмування роботів.

Виходячи з цього сценарію використання, моделювання систем зв’язку на основі AutomationML має охоплювати інформацію про списки I/O, прив’язку змінних (I/O) до комунікаційних пакетів даних, а також параметри пристрою для параметризації комунікаційних пристроїв, такі як адреси (наприклад IP-адреса), адреса доступу до середовища (наприклад MAC-адреса), маски підмереж, адреси шлюзу та використовувані комунікаційні об’єкти, наприклад профільна інформація.

Крім того, моделювання повинно задовольняти такі нефункціональні вимоги. Перелік параметрів пристрою має бути розширюваним користувачами для покриття майбутніх технологій. Потрібно визначити відповідні RoleClassLib та або SystemUnitClassLib, які забезпечують ідентифікацію семантики об’єктів.
