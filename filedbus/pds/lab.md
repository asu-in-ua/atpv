...
- [ ] Створіть проєкт з PLC CODESYS Control Win, в якому:
  ...
  - добавте бібліотеки `ModbusFB` (для комунікаційних функцій) та `Standard` (для таймерів та інших функціональних блоків IEC 61131-3) 
...
- На Google диску створіть папку з назвою `MyLabs`, якщо вона ще не створена, а в ній створіть папку `LabATV`. Посилання на папку `MyLabs` необхідно переслати викладачу для звітності.
- У межах папки `LabATV` розмістіть файл проєкту Machine Expert.
- У межах папки `LabATV` створіть Google документ з копіями екрану та іншими матеріалами, якщо такі потребуються.
