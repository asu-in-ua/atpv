...
## 2. Рівні напруги дискретних сигналів

Дискретні вхідні канали PLC відрізняються за рівнем напруги, типом електричного сигналу та способом його формування. Це означує, з якими датчиками вони сумісні, які пороги значень ON/OFF (ВКЛ/ОТКЛ) мають, яким є споживаний струм та як PLC інтерпретує стан входу.  У стандарті IEC 61131-2 передбачено кілька типів дискретних вхідних сигналів, але найбільш формалізованим і докладно описаним є 24 VDC, разом з тим не обмежується лише ним. У стандарті згадуються й інші рівні напруги, зокрема AC-входи (наприклад, 120 VAC або 230 VAC) та "нестандартні" DC-входи, які можуть працювати на інших значеннях. Виробник повинен сам забезпечити коректність роботи цих інтерфейсів і задекларувати їхні характеристики у технічній документації. Для нестандартних рівнів напруги IEC містить лише загальне розрахункове рівняння, за яким виробник має проектувати такі входи.

### Дискретні входи постійного струму (DC)
...
### Дискретні входи змінного струму (AC)
...
Для коректної взаємодії необхідно, щоб вихід датчика та вхід PLC були протилежних типів: вихід sourcing повинен підключатися до входу sinking, а вихід sinking — до входу sourcing. Приклади підключення sink та source показані на рис.3. 

![image-20251123140800890](mediadido/image-20251123140800890.png) ![image-20251123140817956](mediadido/image-20251123140817956.png)



рис.3. Підключення контакту до дискретного входу sinking (ліворуч)  і sourcing (праворуч)
...
![image-20251123122212862](mediadido/image-20251123122212862.png)

рис.6 Приклад 3-х провідного підключення індуктивного датчика наближення до входу типу sink (+ на вході DI PLC) 

![image-20251123122218509](mediadido/image-20251123122218509.png)

рис.7 Приклад 3-х провідного підключення  індуктивного датчика наближення до входу типу source (- на вході DI PLC)

До недоліків таких датчиків можна віднести наявність додаткового третього проводу.

### Електронні 4-провідні датчики
...
рис.8. Діаграма визначення стану за струмом та напругою
...
Розглянемо на прикладі описі дискретного модуля (рис.9) Modicon BMX DDI 3232.
...
рис.9. Приклад таблиці характеристики дискретного вхідного модулю Modicon BMX DDI 3232 (Система X80 Schneder Electric) 

З таблиці рис.9 можна дізнатися наступну інформацію:
...
## 10. Проєктування системи дискретних входів
...
## 11. Налагодження та діагностика
