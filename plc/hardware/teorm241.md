...
Якщо в конфігурації вибраний вихід Alarm, він встановлюється в логічну 1, коли контролер перебуває у стані RUNNING і прикладна програма не зупинена на точці зупину (breakpoint). Він встановлюється в 0, коли задача зупиняється на точці зупину, сигналізуючи про те, що контролер припинив виконання прикладної програми. Аварійний вихід також встановлюється в 0 у разі виявлення короткого замикання.

Коли контролер з будь-якої причини переходить у стан STOPPED або в один із аварійних (exception) станів, усі локальні виходи (вбудовані та модулі розширення) встановлюються у значення за замовчуванням, означені в прикладній програмі. Для виходів PTO аварійні значення примусово встановлюються в логічний 0 (0 В постійного струму), і ці значення не можуть бути змінені.

Виходи згруповані в кластери максимум по 4 виходи (менше, якщо загальна кількість виходів контролера не є кратною 4):
...
У разі виявлення короткого замикання або перевантаження відповідний кластер із 4 виходів переводиться в стан 0. Можливі два режими роботи стосовно повторної активації:

- автоматичне повторне ввімкнення: одразу після усунення виявленої помилки вихід знову встановлюється відповідно до поточного призначеного йому значення, а діагностичний стан скидається;
- ручне повторне ввімкнення: у разі виявлення помилки її стан запам’ятовується, а вихід примусово встановлюється у значення за замовчуванням доти, доки користувач вручну не скине стан помилки (по відповідній змінній каналу I/O mapping).

Автоматична спробу увімкнення відбувається періодично (від 10 мс до 1 с залежно від моделі).

Про деталі роботи додаткових функцій описано в лекції [Експертні функції контролерів Modicon M241](teorm241expertfn.md)

Для кращого розуміння матеріалу варто зробити [Конфігурування локальної та розподіленої системи I/O M241 в Machine Expert : практична частина](lab_m241.md)
