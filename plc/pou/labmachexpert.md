...
Дана задача має залежність як від входів системи так і від стану. Рішення, яке наведено на рис 2, включає логіку керування виходами, що зав'язана як на стані входів так і виходів, якими керує ПЛК (наприклад Full і Run). Тому для врахування всіх комбінацій, враховуючи що вхідних дискретних сигналів 4, + 2 вихідних сигнали задіяні як входи в логіці необхідно врахувати $2^6$ , тобто 64 комбінацій. Це досить громіздка таблиця, тому необхідно зробити певну декомпозицію. 

Аналізуючи задачу та її рішення можна виділити зв'язні частини:

- $Run = f (Start,Run,Stop)$
- $Standby=f(Run)$
- $Full=f(Level,Full,Phjoto)$
- $Motor=f(Photo,Run,Full)$
- $Solenoid = f(Run,Level,Full,Photo)$ 

Маючи такі однозначні залежності виходів від входів, можна зробити кілька таблиць перевірки, саме там де вони потребуються (для Standby вихід очевидний).   

- [ ] У GoogleSheet `tables` створіть лист `check1`, заповніть таблиці для перевірки виходів за шаблоном таблиці 2, для формування методики перевірки задачі. Замість назв входів та виходів в заголовку таблиці, які зараз взяті з опису в задачі, вставте назви змінних з таблиці 1.

Таблиця 2. Табличний вигляд методики перевірки роботи задачі  

| №    | Run  | Photo | Level | Full | Стан системи         | Solenoid | Примітка |
| ---- | ---- | ----- | ----- | ---- | -------------------- | -------- | -------- |
| 1    | 0    | 0     | 0     | 0    | Всі входи відключені | 0        |          |
| 2    |      |       |       |      |                      |          |          |
| 3    |      |       |       |      |                      |          |          |
| 4    |      |       |       |      |                      |          |          |
| 5    |      |       |       |      |                      |          |          |
...
- [ ] У межах POU створіть Action з назвою `Simplever` на мові Ladder (LD2) та зробіть виклик з POU `Hopper`
...
- [ ] Добавте в Watch List необхідні змінні входів та виходів.
...
- [ ] Зробіть копії екранів для звіту. Сформуйте в папці лабораторної роботи файл гугл документу і розмістіть в ньому копії екранів. 
...
У цій частині лабораторної роботи необхідно реалізувати власну функцію для масштабування. 

- [ ] Розгляньте постановку завдання нижче для функції, яка має виконувати масштабування вимірюваного значення з аналогового входу в інженерні одинці.  
...
- [ ] Проаналізуйте які параметри необхідні для наведеної вище функції, як це розглянуто нижче. 
...
- [ ] Добавте нову функцію з назвою `SCALE` і мовою ST та виходом типу REAL.
...
- [ ] Використовуючи створені таблиці перевірте правильність роботи функції. Якщо вона працює неправильно, поверніться до попереднього пункту і перевірте код на помилки. 

- [ ] Зробіть копії екранів з реалізацією функції, програмою де вона використовується та watch таблиці в якій показані вхідні та вихідне значення. Занесіть це у звіт у гугл документі. 


### 2.5. Експортування функції

- [ ] Через контекстний пункт меню `PLCOopenXMLExport` зробіть експортування функції в окремий файл 
...
Необхідно розробити функціональний блок для керування клапанами або засувкою з пневмоциліндром, що має датчики кінцевого положення. Необхідно реалізувати:
...
- [ ] У проєкті `MyLib` у `Global` створіть новий POU типу функціонального блоку (рис.7)
...
У M241 є кілька варіантів сформувати меандри. Один із них - скористатися функціональним блоком типу `BLINK`. Нижче наведений приклад використання функціонального блоку з іменем `fbBlink`. 

```pascal
fbBlink(ENABLE:= true, TIMELOW:= t#0.5s , TIMEHIGH:=t#0.5s, out=> bA);
```
...
- [ ] У апаратній конфігурації проєкту означте дискретні входи та виходи, які мають контролювати та керувати циліндром а також задавати режим.
...
- [ ] Зробіть копії екранів та таблиці для звітності. Збережіть їх в файлі звіту на Google диску.
...
- [ ] Проаналізуйте оновлені вимоги до початкового задачі, що наведені нижче. 

Задача, наведена на рис.1 має змінений алгоритм роботи. 

По-перше, добавляється датчик та змінюється керування соленоїдом:
...
- замість соленоїда вставляється заслінка, що керуюється пневмоциліндром, має два датчики кінцевого положення, та передбачає можливість ручного керування і формування тривог.
...
Кожен стан машини сигналізується лампами, залежність показана в таблиці 

Таблиця 4. Стан індикаторів в залежності від стану машини (FLASH - миготіння 0.5 секунд)
...
- [ ] Імпортуйте у Global функцію `SCALE` та функціональний блок `Valve`.
...
- [ ] Збережіть проєкт `POU_labs` та розмістіть його на Google Drive в папці лабораторної роботи
- [ ] Відправте посилання викладачу на файл  Google Drive

## 5. Підготовка та відправлення звіту

- [ ] На Google диску у папці в папці `LabPou`, що знаходиться в папці `MyLabs`, вже мають бути створені усі необхідні матеріали, які заповнювалися по ходу виконання роботи.
