...
Літерали — це безпосередньо введені "жорстко закодовані" постійні значення або рядки. Стандарт IEC 61131-3 означує літерали як "зовнішнє представлення даних". Літерали можна вводити безпосередньо в код. Декларацію вказувати не потрібно. Стандарт IEC 61131-3 описує різні літеральні типи відповідно до основних типів даних. У цьому розділі будуть подаватися літерали одразу з типами даних.

У офіційній документації замість слово "літерал" вживається слово "константа". Надалі ми будемо розділяти ці поняття, так як в цьому посібнику під константою розуміється поіменована незмінна область пам'яті.

Загалом, під час використання літералів IEC застосовується найменший можливий тип даних. Винятком є літерал REAL і LREAL, для яких завжди використовується LREAL. Якщо потрібно застосувати інший тип даних, слід використовувати типізовані літерали без необхідності їх явного оголошення. Для цього літерал доповнюється префіксом, який означує її тип.
...
Таб.2. Літерали для цілих типів
...
Типи з плаваючою комою представлені типами REAL і LREAL.

Таб.3. Дійсні типи даних 
...
- у області оголошень програмних елементів POU (Declaration Editor)
- через діалогове вікно `Auto Declare` 
- у редакторі GVL
- у редакторі `PersistentVars`

Оголошення можна робити у табличному вигляді або у текстовому. Текстовий вигляд має більше можливостей, зокрема більш гнучке задавання значень ініціалізації. Користувач може перемикатися між виглядами за допомогою відповідної кнопки в редакторі (`Tabular View` або `Textual View`), якщо такий вигляд доступний (рис.1). 
...
рис.1. Табличний та текстовий вигляд області оголошення змінної POU 

У такому вигляді задаються поля (див. рис. 1):

- `Scope` - область дії змінної; 
- `Name` - назва змінної; 
- `Address` - прив’язка до конкретної адреси  (опціонально)
- `Data type` - тип даних змінної (опціонально); 
- `Initialization` - значення ініціалізації змінної  (опціонально);
- `Comment` - коментар (опціонально); 
-  `Attributes` - атрибути (опціонально);

Призначення цих полів детально розглянуті нижче.

Синтаксис оголошення змінної в текстовому вигляді наступний:

```pascal
<Identifier> {AT <address>} : <data type> {:=<initialization>};
```

Частини у фігурних дужках `{}` є необов’язковими. Для коментарю використовується синтаксис  ST, тобто  `// однорядковий`  та `(* багаторядковий *)`.   Для текстового вигляду область дії змінних (`Scope`) , що оголошуються, задається ключовими словами, які охоплюють оголошення однієї або кількох змінних, що обмежується ключовими словами `VAR` і `END_VAR` .  Наприклад:
...
Ключові слова області дії змінних можуть доповнюватися атрибутами, такими як `RETAIN`, `CONSTANT`, `PERSISTENT` наприклад  `VAR CONSTANT`

### Область дії (Scope)

Область дії (Scope), яка також називається Variable Type (не плутати з типом даних змінної), вказує на те, де змінна є видимою та доступною, а також означує правила її використання і життєвий цикл у межах програми, функціонального блока або всього застосунку. У табличному представлені області об'явлення змінної `scope` є однією з колонок, і для кожної змінної область дії вказується окремо. Для текстового представлення область дії оголошується для однієї або кількох змінних між ключовими словами  `<scope>` (наприклад `VAR_INPUT`) і `END_VAR`,      

У залежності від місця об'явлення змінної можливі наступні варіанти:

- VAR - локальні змінні (local variables), доступні для запису тільки в межах області об'явлення 
...
- VAR_IN_OUT CONSTANT - змінні пересилання (transfer variable), використовується як константний параметр, що передається за посиланням. До такого параметра може передаватися змінна або константа (літерал) типу STRING або WSTRING. Параметр є лише для читання. Передавання властивостей не допускається.
- VAR_GLOBAL - глобальні змінні (global variables), оголошуються тільки в конкретному глобальному списку змінних (GVL), в інших частинах проєкту не допускається їх оголошення. Звернення через `<GVL_name>.<VAR_name>` 
- VAR_TEMP - тимчасові змінні (temporary variables),  які ініціалізуються або повторно ініціалізуються під час кожного виклику POU. Оголошення VAR_TEMP можливе лише в POU і функціональних блоках, і доступні тільки в тілі програмного POU або функціонального блока. VAR_TEMP також можна використовувати замість VAR для зменшення обсягу пам’яті, що використовується POU, наприклад усередині функціонального блока, якщо змінна застосовується лише тимчасово.
- VAR_STAT - статичні змінні (static variables) у функціональних блоках, методах і функціях, ініціалізуються при першому виклику, і зберігають своє значення між викликами. Крім того, вони мають спільну пам'ять для всіх викликів одного і того ж POU або екземпляра функціонального блока. Їх можна застосовувати, наприклад, у функції як лічильник кількості викликів цієї функції.   
...
До оголошення області дії змінної можна додати атрибути, яка вказує можливість збереження змінної при вимкненому живленні (реманентність) та можливість її зміни з програми користувача:
...
На рис.2 показано дві змінні в списку персистентних змінних у верхній частині вікна показано об’єкт `PersistentVars`, у якому означено дві персистентні змінні: одна оголошена безпосередньо в списку, інша створена через шлях екземпляра для змінної, оголошеної з атрибутом `VAR PERSISTENT` у програмі `POU1`. У нижній частині видно локальне оголошення змінної `real2` у програмі `POU1` з атрибутом `VAR PERSISTENT`, для якої система згенерувала відповідний `instance path` і додала його до списку персистентних змінних після відповідної команди. Таким чином, фактична персистентність змінної `real2` забезпечується саме її наявністю в списку `PersistentVars`, а не лише атрибутом `PERSISTENT` у `POU`.

![image-20260205175719841](mediam241/image-20260205175719841.png)

рис.2. Персистентні змінні
...
Забезпечення персистентності здійснюється за такими процедурами:
...
Багаторазове використання одного і того ж імені змінної доступне за наступних правил: 

- локально дублювання ідентифікаторів не допускається
- глобально багаторазове використання ідентифікатора допускається: локальна змінна може мати таке саме ім’я, як і глобальна. У цьому випадку в межах POU пріоритет має локальна змінна
- змінна, визначена в глобальному списку змінних (GVL), може мати те саме ім’я, що й змінна, визначена в іншому глобальному списку змінних (GVL). У цьому контексті слід враховувати такі розширення IEC 61131-3:
  - оператор глобальної області видимості: шлях до екземпляра, що починається з крапки (`.`), відкриває глобальну область видимості. Тобто, якщо існує локальна змінна, наприклад `ivar`, з таким самим ім’ям, як у глобальної змінної, то `.ivar` посилається на глобальну змінну
  - ім’я глобального списку змінних (`GVL`) можна використовувати як простір імен для змінних, що в нього входять. Можна оголошувати змінні з однаковими іменами в різних глобальних списках змінних (GVL). Доступ до них здійснюється шляхом додавання перед іменем змінної імені списку, наприклад у `globlist1.ivar := globlist2.ivar;` `ivar` з `globlist2` копіюється в `ivar` у GVL `globlist1`
  - змінні, визначені в глобальному списку змінних бібліотеки, що підключена, доступні за синтаксисом `<простір імен бібліотеки>.<ім’я GVL>.<змінна>`, наприклад у `globlist1.ivar := lib1.globlist1.ivar` `ivar` з `globlist1` у бібліотеці `lib1` копіюється в `ivar` у GVL `globlist1`
- для бібліотеки також визначається власний простір імен у момент її підключення через Library Manager. Тому доступ до модуля або змінної бібліотеки здійснюється як `<простір імен бібліотеки>.<ім’я модуля | ім’я змінної>`. Слід враховувати, що у випадку вкладених бібліотек простори імен усіх задіяних бібліотек потрібно вказувати послідовно. Приклад: якщо `Lib1` підключена бібліотекою `Lib0`, то доступ до модуля `fun`, що є частиною `Lib1`, здійснюється як `Lib0.Lib1.fun`, що матиме вигляд  `ivar := Lib0.Lib1.fun(4, 5); (* значення, яке повертає fun, копіюється в змінну ivar проєкту *)`; якщо у вікні `Properties` підключеної бібліотеки `Lib` активовано прапорець `Publish all IEC symbols to that project as if this reference would have been included there directly.`, то доступ до модуля fun можливий також безпосередньо як `Lib0.fun`.
- змінні, оголошені в GVL або POU вузла `Global` дерева `Applications`, доступні з додаванням префікса-оператора `__POOL.`.
...
При задаванні адреси пам'яті вкзаується наступна інформація:

- `%` - ідентифікатор адреси згідно IEC 61131-3
- `<memory area prefix>` - область пам’яті ;
- `<size prefix>` - формат пам’яті (розмір);
...
Підтримуються такі префікси областей пам’яті:

- `I` — входи (фізичні входи через драйвер входів, датчики);
- `Q` — виходи (фізичні виходи через драйвер виходів, виконавчі механізми);
- `M` — область пам’яті.

Підтримуються такі префікси розміру:

- `X` — один біт, якщо ;
- `B` — байт (8 біт);
- `W` — слово (16 біт);
- `D` — подвійне слово (32 біти).
- `L` - 64-бітне слово (тільки для CPU що підтримують)

Варто зауважити, що у середовищах Machine Expert для деяких контролерів підтримується long-адресація внутрішньої пам’яті `%M` у вигляді `%ML<n>`, яка використовується для 64-бітних типів даних (LREAL, LINT). Така адресація є реалізаційною особливістю конкретних контролерів і не входить до універсального опису Direct Addresses в офіційній документації.
...
| `%QB7`                 | вихідний байт 7                                   |
| `%MD48`                | подвійне слово в позиції 48 області пам’яті       |
| `ivar AT %IW0 : WORD;` | приклад оголошення змінної з прив’язкою до адреси |

Розмір пам’яті для вхідних, вихідних даних і області пам'яті (оголошення з `AT` `%I`, `%Q` та `%M`) наперед означений цільовим пристроєм, для деяких пристроїв може бути змінений у властивостях об’єкта Application. Пристрої використовують один з двох режимів адресації: по байтам або по словам.
...
Діапазон другого елемента бітової адреси (число після крапки) такий:

- режим адресації по байтам: `0…7`;
- режим адресації по словам: `0…15`.

Обробку бітових адрес також можна налаштовувати на рівні пристрою. У такому разі компілятор Machine Expert інтерпретує їх відповідно до конфігурації. Наприклад: у пристрої з байтовою адресацією бітова адреса `%IX2.5` звертається до байта `2` (`IB2`). У пристрої зі словною адресацією вона звертається до слова `2`, що відповідає іншому розташуванню в пам’яті. Булеві значення виділяються побайтно, якщо явно не задано адресацію одного біта. Наприклад, зміна значення `varbool1 AT %QB7` впливає на діапазон від `QX0.0` до `QX0.7`.

Для PLC M241, M251 перекривання адрес матиме вигляд, як показано в таблиці 1.
...
Це легко перевірити у режимі відлагодження, ввівши адреси з різними типами, які перекриваються (рис.3). Зверніть увагу що поле Address вказує на байтове розміщення в області пам'яті.    
