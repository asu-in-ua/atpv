...
Даний посібник дає стиснуту інформацію щодо запуску і перевірки роботи регулятору PID_Compact V2 з вбудованої бібліотеки регулювання S7 1200/1500.  
...
Дослідження роботи регулятору варто робити разом з об'єктом. Оскільки передбачається, що дослідження проводиться на імітаторі ПЛК, то об'єкт регулювання буде також імітуватися. Найбільш простим способом імітувати об'єкт - використати аперіодичну ланку 1-го порядку. У цьому прикладі икористаємо для цього вбудований функціональний блок `Filter_PT1`  

![image-20241214123908989](media1200/image-20241214123908989.png)

рис.2. `Filter_PT1`  у бібліотеці TIA Portal

`Filter_PT1`  є пропорційним елементом передачі із затримкою першого порядку, який можна використовувати для різних цілей, зокрема: для фільтрації сигналу з метою послаблення високочастотних шумів; згладжування крокових змін сигналу, наприклад зміни уставки або вихідного значення регулятора; ну і моделювання технологічної змінної.

Для  `Filter_PT1`  можна вказати наступні параметри:

- Пропорційне посилення вхідного сигналу `Gain`, який є множником, тому при рівності `1.0` сигнал на виході після завершення перехідного процесу буде дорівнювати сигналу на вході, коли `<1.0` - буде послаблюватися, `>1` - пдісилюватися 
...
Блок `Filter_PT1` має викликатися періодично, тому необхідно створити функціональний блок та викликати його в періодичному OB. У нашому випаду створено функціональний блок  `aiIsml` екземпляр якого `aiIsml_DB` викликається в `OB30`.  

```pascal
"aiIsml_DB"();
```

Надалі будемо вважати що імітується перепад тиску на вентиляторі, оберти якого будемо змінювати. Програма `aiIsml` наведена нижче. 
...
Налагодження цієї частини програми передбачає зміну вхідної змінної об'єкту та перегляду зміни вихідної змінної. Якщо імітується робота операторської панелі, то значення змінної можна вивести на тренд. Після часу ` #LAG1.Lag` вихідне значення буде `#PV:=#Valve*#LAG1.Gain`.  
...
Регулятор PID_Compact в TIA Portal створюється та конфігурується у розділі `Technology objects`. Щоб перевірити його роботу в імітаторі ПЛК, треба зконфіугурувати проект з S7 1500, так як імітатор S7 1200 не підтримує роботу даних функціональних блоків (мені дивна така поведінка, але це факт). Ось як це виглядає в прикладі.   

![image-20241215212628782](media1200/image-20241215212628782.png)

рис.6. Створення екземпляру PID_Compact

Як видно з рис, даний функціональний блок має окремі лінки на налаштування (`Configuration`) та введення в дію та налагодження (`Commissioning`). Конфігурування доступне також через вікно інспектора редактора програмування. 

Виклик екземпляра `PID_Compact` повинен відбуватися тільки в OB циклічного переривання. У нашому випадку для цього створено `OB30` де вже викликається функціональний блок імітатора об'єкта. Тому можна перетягнути технологічний об'єкт в OB30, і налаштувати його через вбудований інспектор (курсор на імені -> вкладка Configuration, рис.7).     
...
У нашому випадку ми запишемо їх як інструкції, вибравши режим масштабованих значень (не `_PER`), поступовим заповненням вікна інспектора ми отримаємо такий код: 
...
У групі властивостей `Controoler type` змінимо тип на `Pressure`. При старті CPU регулятор запуститься в ручному режимі (див. описи режимів нижче), а в `process value limits` виставимо межі вимірювальної величини для регулювання, наприклад від `20 Pa` до `220 Pa`. Також введемо обмеження на вихід регулятора `Output value high limit` рівним `90%`. 
...
Налаштування `Reaction on Error` залишимо без змін, про них буде написано в розділі "Режими роботи".

Викликавши вікно конфігурації можна подивитися додаткові налаштування, а також перейти на закладку з усіма параметрами функціонального блоку.
...
Сам екземплярний DB можна переглянути через пункт `Open DB Editor` контекстного меню PID Cpmpact.

## Перше введення в дію PID_Compact

Після створення базових налаштувань компілюємо та завантажуємо в імітатор ПЛК проект. Після того можна налаштувати регулятор через вікно введення в дію `Commissioning`, яке викликається через відповідний пункт технологічного об'єкту  (рис.12). У цьому вікні включаємо перегляд `Monitor All` та самописець з дискретністю 1 с. 

![image-20241216182912567](media1200/image-20241216182912567.png)

рис.12. Запуск через вікно введення в дію

Для початку перевіримо роботу регулятора в ручному режимі (саме в такому він запустився). Для цього виставимо значення виходу на `80%` і натиснемо `Sends manual mode to the CPU`.  

![image-20241216183304160](media1200/image-20241216183304160.png)

рис.13. Зміна значення виходу в ручному режимі

На самописці ми отримаємо криву розгону (рис.14). Використовуючи кнопки керування самописцем, можна масштабувати графік. 

![image-20241216183845296](media1200/image-20241216183845296.png)

рис.14. Крива розгону отримана в ручному режимі. 

Змінимо значення на `0%` і дочекаємося усталеного режиму. Натиснувши Stop, та знову Run можна перезапустити самописець.

Для запуску регулятора натиснемо необхідно зняти `Manual Mode` і запустити регулятор кнопкою `Strt PID_Compact` (рис.15), а в Watch Table змінимо завдання на SP рівним `125`.  

 ![image-20241216184846592](media1200/image-20241216184846592.png)

рис.15. Запуск автоматичного режиму.

На цьому етапі регулятор досягне свого значення десь через хвилину.  
...
Параметри `PID_Compact` відображаються у вікні конфігурації «PID Parameters». У нашому випадку вони наразі є за замовченням, і матимуть вигляд як на рис.17. 

![image-20241217083431766](media1200/image-20241217083431766.png)

рис.17. Вікно параметрів PID зі значеннями за замовчуванням

Передбачається що ці параметри будуть адаптовані до керованої системи під час авто-налаштування регулятора.  Але їх можна вводити вручну за необхідності. Для PID_Compact V2 параметри знаходяться у структурі `Retain.CtrlParams`, змінювати їх можна в онлайн, але бажано в режимі "Inactive", щоб запобігти помилок ПІД-регулятора. Якщо ви хочете змінити параметри PID в онлайн в режимі "Automatic mode" або "Manual mode" , необхідно змінювати параметри PID таким чином: змінити параметри PID у структурі `CtrlParamsBackUp` і застосувати ці зміни за допомогою `LoadBackUp = TRUE` до структури `Retain.CtrlParams`. Онлайн-зміни параметрів PID в "Automatic mode" можуть призвести до стрибків вихідного значення.
...
При використанні `Output_PWM`, то час дискретизації алгоритму PID використовується як період часу широтно-імпульсної модуляції. Точність вихідного сигналу визначається відношенням часу дискретизації алгоритму ПІД до часу циклу ОВ. Тому рекомендовано, щоб тривалість циклу становила максимум одну десяту часу вибірки алгоритму ПІД.
...
**4 - Manual mode (ручний)**. У цблму режимі вихідне значення задається вручну через `ManualValue`. Рекомендується змінювати цей режим роботи лише за допомогою `Mode` та `ModeActivate`, але його можна активувати за допомогою `ManualEnable = TRUE`. 

Перемикання з ручного режиму на автоматичний здійснюється безударно. Ручний режим доступний навіть при роботі регулятора з помилкою.
...
Отже необхідно забезпечити щоб плинне значення перед запуском попереднього налаштування було: Input < 62.5 Pa. Для цього достатньо в ручному режимі задати на вихід `20%`. Після завершення перехідного процесу `Input = 50.0`. Протягом всього часу в полі `Tuning Status Error` буде світитися помилка виходу значення за діапазон (рис.19). Після досягнення необхідного значення, кнопка підтвердження `ErrorAck` скине помилку, що вкаже на можливість запуску автоналаштування (`Tuning has not been started yet.`). 
...
По завершенню у полі статусу відобразиться `System tuned.` А регулятор перейде в автоматичний режим. Якщо попереднє налаштування не вдалось, перемикання режиму роботи залежить від `ActivateRecoverMode`.

![image-20241219005021745](C:\san\народн посібн атпв\gitver\plc\pidcontrol\media1200\image-20241219005021745.png)
...
Запуск точного налаштування може запуститися з автоматичного режиму. Якщо він запускається з ручного або неактивного режиму регулятору, тоді буде спочатку проведене попереднє налаштування, після чого запуститься точне. 
...
Для завантаження параметрів в проект, необхідно натиснути кнопку `Upload PID Parameters`, після чого дійсні значення стануть значеннями за замовченнями. Далі зміни в проекті треба завантажити в ПЛК.

![image-20241219010305378](media1200/image-20241219010305378.png)

рис.24. Вивантаження налаштованих параметрів в проект 

Для перевірки роботи регулятору, змінимо значення завдання до 60 Pa. Перехідний процес триває близько 10 секунд
...
Для можливості керування регулятором зовнішніми засобами HMI, змінимо програму до наступного вигляду:
