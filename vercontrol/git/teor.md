...
### Поняття гілки, тегу та HEAD в Git

Розглянута вище модель комітів із посиланнями на попередні фіксації природним чином приводить до поняття гілок. Оскільки кожен коміт містить посилання на попередній, у найпростішому випадку формується лінійний ланцюжок фіксацій, який відповідає одній послідовності розвитку проєкту. Кожен новий коміт, створений у межах гілки, зміщує відповідний вказівник на наступний коміт, зберігаючи зв’язок із попередніми фіксаціями через посилання `parent`. Таким чином, **гілка (branch)** у Git є іменованим вказівником на певний коміт у репозиторії, який представляє окрему лінію розвитку проєкту.

У найпростішому випадку репозиторій містить одну гілку, в якій коміти утворюють лінійний ланцюжок. Часто ця гілка за замовчуванням має назву `main` або `master`. Проте модель даних Git дозволяє створювати кілька гілок, які можуть розходитися від спільного коміту, формуючи розгалужену історію змін. У цій лекції практичні сценарії створення та злиття гілок не розглядаються. Водночас важливо розуміти, що сама модель знімків і комітів із посиланнями `parent` уже містить у собі можливість галуження історії проєкту.

На рис. 10 показано вигляд ланцюжка фіксацій з рис. 9 у спрощеному вигляді без деталізації. Цей ланцюжок не має галужень і завершується останнім комітом `f30ab`, який відповідає знімку `Snapshot C`. На нього вказує гілка `master`, що означає, що цей коміт є поточним завершенням історії цієї гілки. При створенні нового коміту в межах цієї гілки вказівник `master` буде автоматично переміщено на новий коміт, продовжуючи лінійну історію змін проєкту.

![Приклад лінійної історії комітів з однією гілкою та вказівником HEAD](media/branch-and-history.png)

рис. 10. Приклад лінійної історії комітів з однією гілкою та вказівником `HEAD` (джерело: git-scm.com)

Поряд із гілками використовуються також теги . **Тег(tag)** є іменованим вказівником на конкретний коміт і застосовується для фіксації певних значущих станів проєкту, наприклад версій або релізів. На відміну від гілок, теги не змінюють свого положення з часом і завжди залишаються пов’язаними з тим комітом, на який були встановлені. На рис. 10 тег `v1.0` також вказує на останній коміт.

Для роботи з історією змін у Git використовується спеціальний вказівник, який називається **`HEAD`**. Він позначає поточний стан проєкту, тобто той коміт, на основі якого сформовано поточний вміст файлів у робочій директорії. У типовому випадку `HEAD` пов’язаний з останнім комітом активної гілки. На схемі рис. 10 вказівник `HEAD` спрямований на гілку `master`, тому поточний стан файлів у робочій директорії сформовано на основі коміту `f30ab`.

Вказівник `HEAD` може бути пов’язаний із будь-яким попереднім комітом у репозиторії. У такому режимі проєкт переводиться у стан, що відповідає вибраному знімку, і всі файли відображаються так, як вони виглядали на момент цієї фіксації. Це дозволяє аналізувати попередні версії проєкту, відтворювати історичні стани та досліджувати еволюцію змін без втручання у поточну гілку розвитку.
...
рис.12. Приклад `git status` в Git Bash
...
У Git Gui стан автоматично показується при запуску, поміщаючи файл у відповідний список `Unstaged` та `Signed Changes` а також відображаючи відповідними піктограмами,  і в даному випадку виглядатиме як на рис.13. Для оновлення інформації про стан треба викликати команду натиснувши кнопку `Rescan` 

![image-20260111112855496](media/image-20260111112855496.png)

рис.13. Приклад `git status` в Git Gui
...
У Git GUI (див.рис.13) користувач може вибрати нові або змінені файли зі списку `Unstaged Changes` та додати їх до області підготовки змін (`Staged Changes`) за допомогою кліку по цьому файлу. Якщо необхідно проіндексувати усі змінені або невідстежувані файли, можна скористатися кнопкою  `Stage Changed`.
...
На рис.14 показаний приклад виклику команди в Git Bash та Git Gui 

![image-20260111114451452](media/image-20260111114451452.png)

рис.14. Перегляд змін у файлах
...
На рис.15 показано як робити коміт в  Git Gui. Для цього, як вже було зазначено, спочатку робиться індексація, а потім вноситься повідомлення в поле Commit Message і натискається кнопка `Commit` 

![image-20260111121756898](media/image-20260111121756898.png)

рис.15.  Коміт в Git Gui 
...
У Git GUI для перегляду історії комітів використовується окремий інструмент візуалізації — gitk, який запускається з меню Git Gui. Він відображає граф комітів, їхні зв’язки та детальні відмінності між версіями. Ту саму інформацію можна отримати в командному рядку Git Bash за допомогою команд `git log` і `git show`, але у текстовому вигляді. На рис.17 показаний приклад перегляду історії за допомогою Gitk, який можна викликати через пункт меню Git Gui  `Reposiory -> Visualise Main History`

![image-20260111123147254](media/image-20260111123147254.png)

рис.17. Перегляд історії за допомогою Gitk

У верхній частині вікна відображено історію комітів гілки `main` у вигляді лінійного ланцюжка. Кожен рядок відповідає окремому коміту і містить:

- назву гілки (`main` вказує на останній коміт);
- короткий коментар коміту (наприклад «Внесені зміни до README»);
- автора коміту;
- дату та час фіксації.

Виділений синім кольором коміт є поточним вибраним комітом, для якого нижче показується детальна інформація. 

У середній частині вікна наведено метадані виділеного коміту:

- повний ідентифікатор коміту (SHA-1 ID);
- автора та комітера;
- час створення;
- посилання на батьківський коміт (`Parent`);
- назву гілки, на яку вказує цей коміт (`Branch: main`);
- зв’язки з попереднім і наступним комітами в історії.

Нижня частина вікна відображає вміст змін (diff), зафіксованих у виділеному коміті. Для текстових файлів показано:

- рядки, які були видалені, позначені символом `-` і підсвічені червоним;
- рядки, які були додані, позначені символом `+` і підсвічені зеленим;
- повідомлення про особливості файлу, наприклад відсутність символу нового рядка в кінці файлу.

Окремо видно, що один файл було скопійовано з іншого (`copy from file1.txt to file2.txt`), що також зафіксовано як частина цього коміту.
