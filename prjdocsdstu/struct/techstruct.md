...
Цей розділ може змінюватися та доповнюватися.
...
- Інструкція з експлуатації КТЗ (позначувався як `ІЕ`)
...
>
> 2) подавати структуру КТЗ автоматизованої системи (за потреби) кількома схемами, першою з яких є укрупнена схема КТЗ автоматизованої системи в цілому.
...
### Інструкція з експлуатації КТЗ(ІЕ)
...
## 3. Схема КТЗ в життєвому циклі

Важливо зауважити що схема КТЗ згідно стандартів ГОСТ 34 (С1) з'являється ще на ранній стадії життєвого циклу АС - ескізного проєкту, або технічного проєкту. Однак практика показує, що часто вона є частиною технічного завдання. Це пов'язано з її відносною простотою та наглядністю, відсутності деталей із-за невизначеністей, які тим не менше можуть з'явитися в схемі пізніше. Цю схему просто "рисувати олівцем" при обговоренні, або наводити у табличному вигляді з прийнятими підходами щодо представлення.      

Нагадаємо, що згідно ГОСТ 34 використовується водоспадна модель ЖЦ, де стадійність робіт не передбачає ітерацій і повернень до ранніх стадій. Таким чином схема КТЗ переходить від стадії до стадії аж до робочої документації, а потім може передатися в експлуатаційну документацію. При цьому вона може змінюватися, або залишатися в первинному вигляді. Нерідкі випадки, що із-за відсутності її потреби при монтажі обладнання, рішення щодо змін, які приймаються на цій стадії (дуже часті випадки), не відображаються в схемі КТЗ. У той же час ця схема слугує досить наглядним орієнтиром для швидкого пошуку і прийняття рішень при усуненні несправностей. Тому якщо вона не відповідає дійсності, то її цінність нівелюється. Хоча це стосується і інших документів супровідної документації і потребує окремого розгляду.         

## 4. Формування вимог до структури КТЗ
...
3. структуру КТЗ можна подавати кількома схемами, першою з яких є укрупнена схема КТЗ автоматизованої системи в цілому (згідно 4.1.2)
...
## 5. Приклади виконання від спільноти

Як вже зазначалося вище, жорстких вимог до схеми КТЗ навіть в старих стандартах ГОСТ 34 немає, тоді як варто нагадати, що ці стандарти навіть не є чинними в Україні. Тому виконання схем може бути зовсім різним, але маючи відповідні технічні компетентності будь хто має її прочитати. Тому задачею мінімум таких схем в тому, щоб їх можна було правильно прочитати, іншими словами вірно зрозуміти, що саме хотів передати автор схеми. У той же час задача максимум - відобразити усю необхідну інформацію на схемі, без читання опису КТЗ, при цьому не перевантажуючи її деталями.  

У цьому підрозділі розберемо кілька прикладів виконання, надані спільнотою автоматників. 
...
Для початку розглянемо схему повністю, щоб означити основні частини документу (рис.Р1). Документ включає:
...
рис.Р1. Загальний вигляд схеми КТЗ (приклад 1)

Тепер розглянемо одну з частин схеми. На рис.Р2 показаний фрагмент схеми, де зокрема зображено:
...
рис.Р2. Фрагмент схеми КТЗ (приклад 1)

Для кращого розуміння даної схеми КТЗ , потрібен опис, так як не вся інформація доступна безпосередньо зі схеми. 

Нижче наведені коментарі розробника (Богдан Вара) щодо зауважень на структуру та призначення схеми КТЗ

> 1. **Про ступінь формалізації схеми**. Під час розроблення цієї схеми не ставилося за мету глибоке опрацювання всіх технічних деталей. Практика показує, що даний тип схем допускає доволі вільне трактування і виконання, тому при її формуванні свідомо було допущено певну умовність і спрощення. Основний акцент робився не на формальній відповідності правилам оформлення, а на доцільності та зрозумілості представлення.
> 2. **Про призначення схеми та контекст її використання**. Схема створювалася на ранньому етапі реалізації проєкту і відображає той цикл проєктування, який склався в конкретній організації; в інших компаніях підхід може бути іншим. Насамперед ця схема призначена для візуалізації обсягу автоматизації для замовника. На практиці не завжди є можливість погоджувати рішення з фахівцями КВПіА, а інколи навіть наявні фахівці не працюють безпосередньо зі схемами автоматизації. Тому схема в даному випадку виконує подвійну функцію: по-перше, як інструмент комунікації з замовником і пояснення того, що саме пропонується в межах автоматизації, і лише по-друге – як елемент проєктної документації.
> 3. **Про рівень деталізації на початковому етапі**. Оскільки схема розробляється на початковій стадії, на цьому етапі ще не визначені конкретні типи кабелів, пристроїв, діапазони датчиків та інші технічні параметри. Усі зображені з’єднання є умовними функціональними зв’язками з вказанням типу сигналу. Більш глибокого технічного змісту в поясненні кожної лінії не закладалося. Використання стрілок і спрощених форм має на меті лише полегшити сприйняття. На цьому етапі також не визначається топологія у строгому сенсі (магістральна чи інша); відомо лише, що між шафами передбачаються кабельні з’єднання, а не повноцінні мережеві сегменти.
> 4. **Про групування елементів на схемі**. Групування елементів виконано з практичних міркувань. Такий підхід спрощує перевірку того, чи включено до проєкту всі необхідні елементи. У реальній роботі ця схема часто використовується безпосередньо під час обговорень із замовником і слугує робочим інструментом для прийняття рішень у форматі «робимо / не робимо» щодо окремих функцій або вузлів.
> 5. **Про зонування та просторове виділення**. Наведений приклад схеми відповідає відносно невеликому об’єкту, тому детальне зонування за допомогою штрихування не застосовувалося. Виділено лише приміщення щитової або операторської. В інших проєктах зонування може виконуватися за принципом належності до технологічних ліній, коли одна лінія може охоплювати кілька фізичних локацій.
> 6. **Про подальше уточнення та актуальність схеми**. Формально схему можна і доцільно було б насичувати додатковою інформацією у міру прояснення технічних деталей. Проте на практиці після виконання своєї основної функції на ранньому етапі проєкту цей документ зазвичай більше цікавить лише проєктанта, який рідко повертається до його актуалізації, оскільки надалі вже працює з більш детальними документами.
> 7. **Про зауваження до умовних позначень**. Зауваження щодо необхідності відображення кросових шаф (на схемах вони позначені як «бокси») в переліку умовних позначень є обґрунтованими і можуть бути враховані при доопрацюванні схеми.
> 8. **Загальна позиція щодо підходу до оформлення**. Загалом підхід до побудови цієї схеми свідомо орієнтований на інформативність і зрозумілість для замовника, навіть якщо це йде врозріз із суворим дотриманням формальних правил виконання схем. На даному етапі така пріоритетність вважається виправданою.

### Приклад 2

У цьому прикладі ми розглянемо наступну схему для автоматизованої системи мікропроцесорної централізації ([тут доступна повна версія](https://drive.google.com/file/d/1YTLcfvcPbShFR83bOYYbPpuFFZW5zKYf/view?usp=drive_link)), спроєктованою в організації Railwayautomatic (RWA), провідним інженером-проектувальником Головатим Степаном, який на даний час боронить нас в Збройних Силах України.   

На рис.Р3 показаний загальний вигляд повної схеми. Для кращого відображення на самому рисунку не показані частини, де наведені умовні позначення та прийняті скорочення, натомість умовні позначення наведені нижче сехми. 
...
рис.Р3. Загальний вигляд схеми КТЗ (приклад 2)
...
Усі польові засоби та засоби перетворення згруповані наступним чином (рис.Р4):
...
рис.Р4. Фрагмент схеми КТЗ  (приклад 2, польові засоби)

Така реалізація дає можливість зрозуміти, як зв'язані польовий рівень та рівень керування, та які проміжні групи засобів використовуються. Зі схеми не видно на скільки багато засобів в групах. Однак якщо навіть припустити що в кожній групі їх по два (що вочевидь надзвичайно занижене припущення)  подумайте, на скільки б ускладнилася дана схема. Виходом з ситуації може бути додаткова схема КТЗ, яка буде показувати тільки частину системи, наприклад для кожного `IM RIO` окремо, якщо звісно в цьому є сенс.

Тепер розглянемо рівень контролерів, який показано на рис.Р5. Зі схеми чітко видно топологію мережі RIO на базі Ethernet, яка об'єднує в кільце наступні засоби:
...
рис.Р5. Фрагмент схеми КТЗ (приклад 2, рівень контролерів)  

Наступна частина - рівень АРМ (рис.Р6). Зі схеми чітко видно структуру мережі верхнього рівня на базі Ethernet, яка побудована з використанням комутаторів за топологією "дерево", до якої входять: 
...
рис.Р6. Фрагмент схеми КТЗ (приклад 2, рівень АРМ)  

Нижче наведений коментар від розробників.

> АРМ залізничних ваг існуючий, через існуючу мережу підприємства підключається до комутатора шафи зв'язку, який проектується в іншому розділі цього проекту. На це вказує примітка 1), де наведено той самий цифровий шифр, що вказаний в основному напису даного креслення.

### Приклад 3

На цьому місці може бути Ваша схема. 

## 6. Авторський погляд

У цьому підрозділі наводиться авторський погляд на виконання структурних схем. Перш за все, варто зауважити, що наведене нижче бачення та приклади враховують академічні цілі, тобто схеми КТЗ використовуються в курсовому та дипломному проєктуванні, а також на практичних заняттях по проєктуванню промислових мереж. Окрім того, ці схеми стосуються виключно мережних структур. Враховуючи це, можна виділити наступні цілі КТЗ:

- відображення рішення щодо мережної структури системи та мережних компонентів
- демонстрація здобувачем своїх компетентностей у:
  - розумінні основи відповідних мережних технологій
  - вибору технічних засобів для рішення
  - системного бачення рішення (у зв'язку з іншими частинами проєкту)   

Враховуючи це, наведений вище список рекомендацій щодо відображення графічних та текстових умовних позначень на схемах КТЗ дещо змінюється:

1. структуру КТЗ можна подавати кількома схемами, першою з яких є укрупнена схема КТЗ автоматизованої системи в цілому;
2. схема відображає мережну структуру системи, або її частини, зв'язки з дискретними, аналоговими, імпульсними типами сигналів не показуються;   
3. на схемі показується мережні засоби та мережні зв’язки між ними, інші (не мержні) компоненти вказуються за необхідності; 
4. дозволяється графічно виділяти (наприклад штриховою лінією) розміщення КТЗ на об’єктах (поверх, цех, тощо) та у виробничих приміщеннях, відповідно до різних вимог техніки безпеки та дотримання ТУ (ex зона, хім виконання тощо);
5. усі мережні засоби повинні унікально ідентифікуватися принаймні в межах схеми, а ще краще в межах проєкту;
6. усі інтерфейси підключення повинні унікально ідентифікуватися в межах технічного засобу;
7. необхідно показувати технічні засоби перетворення даних та підсилення (репітери, адпетри інтерфейсів, комутатори, мости, маршрутизатори, шлюзи); 
8. не варто вказувати пасивні мережні компоненти (маркування кабелів, клемні колодки, вилки/розетки, тощо), так як ця частина показується на схемах з'єднань та підключень, для монтажу;
9. основні характеристики технічних засобів наводяться у таблиці умовних позначень або у відомості мережних технічних засобів;
10. зв'язки між технічними засобами є фізичними (провідники, кабелі), а не логічними чи інформаційними;
11. кожна мережа повинна унікально ідентифікуватися в межах всієї системи;
12. мережні з'єднання показуються у вигляді відрізків, без стрілок; 
13. сегмент мережі можна показувати у вигляді шини з відгалуженнями, навіть якщо фізично підключення відбувається без відгалужень;
14. кожен канал зв'язку має бути ідентифікований відносно способу обміну даними;

### Приклад А1

На рис.А1 наведений приклад схеми КТЗ, який використовується як навчальна демонстрація для виконання у [курсовому проєктуванні в НУХТ](https://github.com/pupenasan/pkis/blob/main/kurs2025/6_2.md#623-структура-ктс ). Приклад містить вже застарілі засоби автоматизації, однак для прикладу це не має значення (тим більше її не варто копіпастити в свої проєкти :-) ). Під рисунком в таблиці А.1 наведена відомість мережних технічних засобів. Як і в наведених вище прикладах, схема має ієрархічний вигляд:

- верхній рівень - диспетчерське керування 
- середній рівень - рівень SCADA/HMI та контролерів
- нижній рівень - засоби розподіленого керування 

Пунктиром виділені місця розміщення технічних засобів: "Диспетчерська", "Відділення дефекосатурації", "Фільтр 1". 

Засоби верхнього рівня об'єднуються в мережу Ethernet, яка позначена як `M1` з використанням одного комутатора `SW1`. Мережні карти ПК позначені як `LAN`. Пристрої SCADA/HMI підключаються до відповідних PLC в режимі точка/точка мережами `M3`, `M4`, `M5`, `M6`. Показані комунікаційні порти, через які відбувається підключення засобів автоматизації. Мережа `M3` передбачає наявність адаптера перетворювача (КАМ). Мережа `M2`  (Modbus RTU) має шинну топологію і базується на інтерфейсі RS-485.          

![image-20250508184346547](media/image-20250508184346547.png)

Рис.А1. Приклад схеми КТЗ (навчальний)  

Таблиця А.1. Відомість мережних технічних засобів.
...
| ПЛК  САТ                     | Програмований  логічний контролер відділення очистки         | 1     | TSX Premium з комм.  картою Modbus RS-485 TSX SCP 114        |
| ПЛК ФІЛ1, ПЛК ФІЛ2, ПЛК ФІЛ3 | Програмований  логічний контролер фільтрації відділення очистки | 3     | вже експлуатується  TWD LCAA 40DRF з ком. модулем   TWD NOZ 485 T |
...
1. Схеми надані представниками організацій, які вказані в прикладах.
