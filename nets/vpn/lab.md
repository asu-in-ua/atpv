...
- `R` - маршрутизатор з вбудованими функціями OpenVPN; у даній лабораторній роботі буде використовуватися Teltonika RUT, однак замість неї можна використовувати маршрутизатори з аналогічною функціональністю;
...
Червоним на рис.1. показані внутрішні IP-адреси кінцевих точок для тунелів. На рис.1 також показаний комп'ютер з якого відбувається налаштування, `ПК адміністратора` - це окремий ПК, який відокремлена від `ingstation`. 
...
- розгортання OpenVPN на клієнтах

## Послідовність виконання роботи

### 1. Корисні посилання

- [ ] Ознайомтеся з посиланнями, які періодично прийдеться переглядати:
...
#### Відкриття оболонки від імені суперкористувача 
...
#### Завершити процеси з іменем `openvpn` 
...
Передбачається, що у вас є віртуальна машина в хмарі або реальний ПК з Debian або Ubuntu, з виділеним IP. Деталі роботи з OpenVPN в Debian описані в [цій статті](openvpndebian.md) 

- [ ] Підключіться до серверу через консоль і встановіть пакет openvpn. 
...
- 10.10.1.2 - клієнтська сторона тунелю, інженерна станція, яка виконується на Windows 10

- [ ] Переключіться в режим адміністратора

```
sudo -s
```

- [ ] Створіть папку `vpnapp1` з конфігурацією та перейдіть в неї.
...
### 5. Запуск openvpn з вказаною конфігурацією
...
### 6. Перевірка прослуховування та доступності порта ззовні
...
- [ ] Якщо порти закриті, то відкрийте порти, які планується використовувати для тунелів, у цьому прикладі це буде порт `11194` .  Про це можна почитати в [цій статті](openvpndebian.md) 
...
- [ ] Завантажте файл `*.msi`  [зі сторінки завантаження](https://openvpn.net/community-downloads/) і запустіть процес інсталяції. Дочекайтеся, коли інсталяція завершиться.
...
- [ ] Зайдіть в папку `C:\Program Files\OpenVPN\config` , створіть папку з іменем `vpnapp1`. Перенесіть туди створений файл.
...
- [ ] Дочекайтеся коли відбудеться підключення або виникне помилка. У першому випадку вийде, вікно з журналом закриється, натомість з'явиться  повідомлення:

![image-20250220155952179](media/image-20250220155952179.png)

рис.5.  

У другому випадку в журналі з'явиться помилка.
...
- [ ] У Windows запустіть командний рядок, перевірте доступність IP тунелю серверу з клієнту:

```
ping 10.10.1.1
```

- [ ] На сервері перевірте доступність IP тунелю клієнта
...
Створення тунелю в попередніх пунктах потрібне для початкових кроків, але це має велику кількість недоліків. Надалі будуть розглядатися кроки для створення захищеної VPN системи, яка описується в загальній постановці задачі. А саме:
...
### 10. Створення центру сертифікації CA

- [ ] Вийдіть з режиму суперкористувача, після чого перейдіть на домашню директорію
...
У процесі розпукування виведе наступну іфнормацію
...
- [ ] Зробіть копію файлу з новим іменем
...
### 11. Створення порожнього списку відкликаних сертифікатів
...
### 12. Створення серверного сертифікату
...
### 13. Створення сертифікату для клієнтів
...
- спочатку треба придумати і ввести PEM для сертифікату клієнту `passphrase` і підтвердити її, **запам'ятайте цю фразу!** 
- потім явно ввести `yes`, коли попросять
- потім задати секретну фразу CA з пункту 10 для підписання сертифікату серверу 
...
### 16. Копіювання файлів сертифікату з серверу
...
- [ ] Завантажте та інсталюйте на робочий ПК (з Windows) WinSCP як вказано на [цій сторінці](https://winscp.net/eng/docs/lang:uk) 
...
### 17. Зміна та перезапуск конфігурації на клієнті Windows

- [ ] Запустіть комп'ютер або віртуальну машину з клієнтом інженерної станції (`ingstation`). 
- [ ] Скопіюйте файли сертифікатів в папку `C:\Program Files\OpenVPN\config\vpnapp1`

- [ ] Змініть файл конфігурації `ingstation.ovpn` наступним чином
...
- [ ] На сервере відкрийте файл конфігурації для редагування
...
- [ ] За допомогою nano створіть CCD файл для `remotesys1`
...
### 21. Перезапуск служб oepnvpn на сервері та `ingstation` з новою конфігурацією
...
### 22. Налаштування клієнта для `remotesys1`

У даному пункті необхідно створити конфігурацію для клієнта `remotesys1` та налаштувати його. Це маршрутизатор Teltonika, який на рис.1 показано літерою `R`. Можуть використовуватися інші типи маршрутизаторів, які мають можливості роботи з OpenVPN.

- [ ] На ПК адмінстратора створіть файл `client.opnvpn` впиіть туди наступний текст (зверніть увагу на необхідність вказівки IP вашого серверу):
...
- [ ] Перейдіть на закладку конфігурації OpenVPN, створіть новий `instance` з роллю `client` та налаштуйте щоб вона завантажувалася з файлу (рис.8) та:
...
- [ ] Підключіть до локальної мережі маршрутизатора `R` якийсь пристрій або віртуальну машину, які відповідають на ping запити
