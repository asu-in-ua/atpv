...
Для шифрування трафіку в тунелі використовується бібліотека шифрування OpenSSL, а також протокол TLS, і міститься багато функцій безпеки та керування. OpenVPN використовує спеціальний протокол безпеки, який базується на SSL/TLS для обміну ключами. OpenVPN у порівнянні з деякими іншими рішеннями здатний проходити транслятори мережних адрес (NAT) і брандмауери. 
...
5) Конфігуруються файли конфігурацій для всіх служб OpenVPN, служби запускаються в роботу.       
...
На рис.1 показаний приклад використання OpenVPN для віддаленого налагодження об'єктів автоматизації. Рішення повинно забезпечити доступ віддаленої станції `Інженерна станція` до мережі будь якого з об'єктів віддаленої підсистеми за необхідності. Передбачається що доступ до мережі клієнта відбуватиметься через маршрутизатор (позначено як `R`), який буде маршрутизувати весь клієнтський трафік через Internet до інженерної станції і у зворотній бік.    
...
![openmaniak openvpn tunnel picture](C:\san\народн посібн атпв\gitver\nets\vpn\media\openvpn_intro1.gif)
...
![image-20250131203016850](C:\san\народн посібн атпв\gitver\nets\vpn\media\image-20250131203016850.png)
