...
# Протоколи та утиліти IP: практична частина з VM Win10
...
Рис. 1. Функціональна схема лабораторної установки
...
- [ ] Ознайомтеся з можливостями налаштування мережі у додатку 1 
- [ ] Виконайте пункти 1-3 та 6 з [практичної роботи по Ethernet](../ethernetstart/labwin10.md), якщо Ви цього ще не зробили.
...
Табл.1. Налаштування IPv4 для мережних карт
...
- [ ] Ознайомтеся з протоколом ICMP у додатку 2.
- [ ] Налаштуйте гостьову та хостові ОС таким чином, щоб вони реагували на ехо-запити (відлуння) ICMP

Деякі вузли не реагують на ехо-запити PING. Наприклад замовченням вони часто відключені у Winidows.  Для активації їх в Winidows необхідно активувати цю опцію в налаштуваннях брандмауера (Рис.2):

![image-20221001201653701](media/image-20221001201653701.png)

 Рис.2. Увімкнення реагування на запит відлуння у Windows 10
...
![image-20260505111428454](media/image-20260505111428454.png)

рис.3. Вузол 192.168.1.1 досяжний

![image-20260505111615788](media/image-20260505111615788.png)

рис.4. Вузол 192.168.1.2 недосяжний
...
- [ ] На карті віртуальної машини, що зв'язується з хостовою ОС через віртуальну мережу зробіть наступні налаштування статичних IP адрес (приклад на рис.5):

- першу статичну адресу, таку, яка була до цього в цієї карти (див.таб.1)
- другу статичну адресу, що відрізняється номером підмережі

![image-20260505112240930](media/image-20260505112240930.png)

рис.5. Приклад налаштування 2-х статичних IP адрес на одній мережній карті
...
- [ ] Ознайомтеся з утилітою TRACERT у додатку 1
...
- [ ] Ознайомтеся з утилітами `Route` та `NetStat` у додатку 2 
...
- [ ] Ознайомтеся з утилітою `nslookup` у додатку 1
...
### Додаток 1. Вбудовані засоби WINDOWS для конфігурування та перевірки роботи IP та TCP
...
Для налаштування параметрів роботи мережної карти та її діагностики можна скористатися графічними засобами провідника Windows. Для різних версій Windows вигляд та доступ до вікон може відрізнятися. Для налаштування властивостей мережних карт у Windows 10 необхідно викликати вікно мережних підключень одним із способів:

- через меню Пуск (Start) → Налаштування (Settings) → Мережа та Інтернет (Network & Internet) → Стан (Status) → Змінити параметри адаптера
- за допомогою команди в командному рядку (Win + R)  `ncpa.cpl`. Після відкриття вікна «Мережні підключення» необхідно обрати потрібний мережний адаптер (Ethernet або Wi-Fi) та відкрити його властивості через контекстне меню.

![image-20260505113201873](media/image-20260505113201873.png) ![image-20260505113235023](media/image-20260505113235023.png)

рис.6.

Налаштування роботи карти та драйверу Ethernet проводиться через кнопку "Configure...". Зокрема на вкладці "Advanced" (рис.7) можна налаштувати швидкість передачі та тип з’єднання (LinkSpeed/DuplexMode), вручну виставити MAC-адресу (Locale Administrated Address), налаштувати параметри мічення кадрів Priority & VLAN (802.1Q/1p VLAN Tagging).  

![image-20260505114612539](media/image-20260505114612539.png)

Рис.7. Вікно додаткових властивостей мережної карти

У списку компонентів для вказаного підключення вказані служби та драйвери протколів, які інстальовані для нього. Для мережних карт Ethernet (а також IEEE 802.11 WiFi) на комп’ютері з ОС Windows, разом з системою автоматично інсталюються драйвери TCP/IP, які можна налаштувати нажавши кнопку "Properties", після вибору даного пункту. 

![image-20260505114948992](media/image-20260505114948992.png)

Рис.8. Налаштування протколу TCP/IP.

Основні властивості протколу TCP/IP налаштовуються у вікні "General" (рис.8). IP-адресу можна задати статично ("Use the foolowing IP address") або вказати динамічний режим ("Obtain an IP address automaticaly"), при якому адреса буде видаватися DHCP-сервером. При статичній адресації крім самої IP-адреси треба задати маску підмережі, та адресу основного шлюзу (gateway), при необхідності міжмережного з’єднання. У налаштуваннях вказується також два первинні DNS-сервери, через які проводиться отримання адрес по доменному імені. 

Для розширеного налаштування протоколу необхідно визвати вікно додаткових параметрів (кнопка "Advanced").

Для відображення стану роботи мережі можна скористуватися кнопкою "Status" в контекстному меню вибраного підключення. Там вказується активні налаштування IP, МАС-адреса, швидкість і т.д (рис.9). 

![image-20260505115253021](media/image-20260505115253021.png)

Рис.9 Вікна відображення стану роботи мережі.

Для Windows 10 до налаштування карти та відображення стану можна зробити через панель керування або провідник, наприклад як це показано на рис.10.

![image-20221001143607786](media/image-20221001143607786.png)

 Рис.10. Доступ да налаштувань мережної карти Windows 10
...
```
  ...
  Таб. 2. Параметри команди ipconfig.
...
```
  ...
  Таб.3. Параметри команди tracert.
...
Таб.5. Параметри команди netstat.
...
Рис.11. Структура кадрів IEEE 802.3 та Ethernet II
...
До даних верхнього рівня реалізація протоколу IP добавляє заголовок, поля якого показані на рис.12.

![image-20221001190900460](media/image-20221001190900460.png)

Рис.12. IP-датаграма, поля IP-заголовку.
...
Таб. 6. Інформація в таблиці маршрутизації.
...
Підтримка протоколу ICMP обов’язкова при реалізації стеку TCP/IP. ICMP-пакети інкапсулюються в IP-пакети (рис.13). Теперішня версія ICMP для IPv4 називається `ICMPv4`, для IPv6 - `ICMPv6`. 

![image-20221001191007516](media/image-20221001191007516.png)

Рис.13. Формат повідомлення ICMP.
...
На рис.14 показана структура пакету, який використовується в запитах і відповідях ARP. У мережах Ethernet в цих пакетах використовується EtherType 0x0806, та розсилається широкомовно MAC-адреса — FF:FF:FF:FF:FF:FF. 
...
Рис.14. Формат повідомлення ARP.
