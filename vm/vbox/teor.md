...
- Одночасного запуску кількох ВМ зі різними операційними системами на одній хостовій ОС. 
...
### Керування VirtualBox через командний рядок

VirtualBox має графічний користувацький інтерфейс для керування віртуальними машинами. Більшість дій, які виконуються через графічний інтерфейс, також можна виконувати з командного рядка. Для цього використовується утиліта VBoxManage.

![image-20260421093324805](media/image-20260421093324805.png)

рис.4.1. Команди для машини з меню 

Командний рядок зручний для автоматизації, створення скриптів розгортання лабораторій або керування великою кількістю віртуальних машин.

#### Перегляд віртуальних машин

Команда показує всі зареєстровані в системі віртуальні машини.

```
VBoxManage list vms
```

#### Перегляд запущених віртуальних машин

Команда відображає лише ті машини, які зараз працюють.
...
#### Запуск із графічним інтерфейсом

```
VBoxManage startvm "VM_NAME" --type gui
```

#### Запуск без графічного інтерфейсу (headless режим)

```
VBoxManage startvm "VM_NAME" --type headless
```

Headless режим часто використовується на серверах або у випадках, коли віртуальні машини запускаються автоматично.

#### Зупинка віртуальної машини

М’яке вимкнення через ACPI (аналог натискання кнопки живлення):
...
#### Пауза та відновлення роботи

Пауза віртуальної машини:
...
#### Snapshot (знімок стану машини)

Snapshot дозволяє зафіксувати повний стан віртуальної машини (диски, пам’ять, конфігурацію) у певний момент часу.

Створення snapshot:

```
VBoxManage snapshot "VM_NAME" take "snapshot1"
```
...
Відновлення snapshot:

```
VBoxManage snapshot "VM_NAME" restore "snapshot1"
```

Видалення snapshot:

```
VBoxManage snapshot "VM_NAME" delete "snapshot1"
```

Snapshot часто використовують під час тестування або навчальних лабораторних робіт, щоб швидко повернути систему до початкового стану.

#### Перегляд інформації про машину

```
VBoxManage showvminfo "VM_NAME"
```

Команда відображає детальну інформацію про конфігурацію віртуальної машини: процесори, пам’ять, диски, мережі.

#### Зміна ресурсів віртуальної машини

Зміна кількості процесорів:

```
VBoxManage modifyvm "VM_NAME" --cpus 2
```

Зміна обсягу оперативної пам’яті:

```
VBoxManage modifyvm "VM_NAME" --memory 4096
```

#### Налаштування перенаправлення портів (NAT)

Приклад створення правила для доступу по SSH:

```
VBoxManage modifyvm "VM_NAME" --natpf1 "ssh,tcp,,2222,,22"
```

Після цього до гостьової системи можна підключатися через:
...
```
VBoxManage clonevm "VM_NAME" --name "VM_CLONE" --register
```

Команда створює копію існуючої машини та одразу реєструє її у VirtualBox.

#### Експорт віртуальної машини

Експорт у формат OVA:

```
VBoxManage export "VM_NAME" -o vm.ova
```

Цей формат дозволяє переносити віртуальні машини між різними системами.
...
Команда імпортує віртуальну машину з файлу OVA.

#### Практичне використання snapshot

Часто перед експериментом створюють контрольний snapshot:

```
VBoxManage snapshot "VM_NAME" take clean
```

Після завершення експерименту машину можна повернути у початковий стан:

```
VBoxManage snapshot "VM_NAME" restore clean
```

Це значно пришвидшує проведення лабораторних робіт і тестування програмного забезпечення.
