...
![image-20260323080054312](media/image-20260323080054312.png)

рис.1. Приклад розподіленого IIoT застосунку

За необхідності налагодження та супроводження подібних до рис.1 рішень також виникає потреба в формуванні відповідного середовища тестування. Для цього можна наприклад використати віртуальні машини (рис.2), які будуть емулювати конкретне середовище розгортання разом з застосунком на ньому.  Однак необхідність віртуалізації ОС приводить до значних затрат ресурсів хостової машини, а для масштабних систем може взагалі бути нереальним. 

![image-20260323130848320](media/image-20260323130848320.png) 

рис.2. Використання віртуальних машин в якості середовища тестування

Контейнери є одним із способів вирішити цю проблему. Ідея полягає в тому, що кожен сервіс постачається разом зі своїм оточенням: потрібними бібліотеками, утилітами та налаштуваннями. Контейнерні рушії забезпечують щоб це все працювало поверх ОС. Це дозволяє запускати той самий застосунок однаково на ноутбуці розробника, на сервері, в хмарі та на edge-вузлі, не змінюючи сам застосунок і не «підлаштовуючи» операційну систему під кожен проєкт, бо за взаємодію з ОС відповідає рушій, який робить процеси інтеграції прозорими.

![image-20260322212310252](media/image-20260322212310252.png)

рис.3. Ідея контейнеру.

На відміну від віртуальних машин контейнери працюють поверх існуючого ядра, використовуючи механізми ізоляції процесів, файлових систем і мереж в контейнері. За це відповідає рушій, який не замається віртуалізацією ОС, а забезпечує правильне керування ресурсами. Завдяки цьому контейнери стартують швидко, споживають менше ресурсів і добре підходять для сценаріїв, де важлива масштабованість і відтворюваність середовища. У цьому відношенні вони мають перевагу над віртуальними машинами.
...
рис.4. Віртуальні машини та контейнери ([Джерело](https://systemadminspro.com/overview-of-container-sandbox-technologies/))
...
- **Контейнер (container)** це процес, запущений в ізольованому середовищі з чітко означеними межами доступу до ресурсів. Саме це поєднання ізоляції та спільного ядра ОС (shared OS kernel) є ключовою відмінністю контейнерного підходу.

З інженерної точки зору контейнер зручно сприймати як стандартну «обгортку» для запуску сервісу. Всередині контейнера завжди відомо, які бібліотеки, утиліти та конфігурації доступні, незале'жно від того, де він запущений. Це дозволяє описувати систему через формалізований опис того, що і в якому вигляді має працювати. Саме з цього базового поняття далі випливають образи, життєвий цикл контейнерів, робота з даними та мережами.

## 3. Контейнер всередині

Контейнери побудовані на поєднанні кількох можливостей операційної системи Linux, які розвивалися задовго до появи Docker. Основна ідея полягає в тому, щоб обмежити, що саме процес може бачити і скільки ресурсів він може використовувати.  

У Linux процес і потік є **задачами (task)** ядра, які можуть за необхідності ділити ресурси між собою. **Процес** - це задача з власним адресним простором, а **потоки** це задачі в межах спільного адресного простору. Саме ця модель дозволяє реалізувати контейнерну ізоляцію на рівні ядра без введення нових типів об’єктів.
...
рис. 5. Простори імен для мереж [Джерело](https://iximiuz.com/en/series/mastering-container-networking/)
...
У підсумку контейнер це узгоджене використання просторів імен, обмежень ресурсів і файлової ізоляції. 
...
рис.6. Про Docker ([Джерело](https://becominghuman.ai/docker-containers-what-are-they-how-to-create-and-use-them-5f03c7260555))
...
### Шарова модель контейнера

Контейнерні образи є шаблонами, з яких створюються контейнери. Ці образи не складаються з одного монолітного блоку, а побудовані з багатьох шарів. Перший шар образу також називається базовим шаром. Це показано на наступному рисунку.

![image-20260322201941308](media/image-20260322201941308.png)

рис.7 Образ як стек шарів

Кожен шар містить файли та каталоги. Кожен шар містить лише зміни файлової системи відносно шарів, що знаходяться під ним. Щоб створити віртуальну файлову систему з набору шарів Docker використовує Union filesystem. Драйвер зберігання відповідає за деталі того, як ці шари взаємодіють між собою. Існують різні драйвери зберігання, які мають свої переваги та недоліки в різних ситуаціях.

Усі шари контейнерного образу є незмінними (immutable). Незмінність означає, що після створення шар ніколи не може бути змінений. Єдина можлива операція, що може вплинути на шар — це його фізичне видалення. Така незмінність шарів важлива, оскільки відкриває дуже багато можливостей. На наступному рисунку показано, як може виглядати власний образ для вебзастосунку, що використовує Nginx як вебсервер.

![image-20260322202607407](media/image-20260322202607407.png)

рис.8. Приклад власного образу на основі Alpine та Nginx

У цьому випадку базовий шар складається з дистрибутива Alpine Linux.  Alpine Linux — це мінімалістичний дистрибутив Linux з малим розміром і підвищеною безпекою, який часто використовується як базовий образ для контейнерів Docker. Поверх нього знаходиться шар Add Nginx, у якому Nginx додається до Alpine. Нарешті, третій шар містить усі файли, що утворюють вебзастосунок, такі як файли HTML, CSS та JavaScript.

Як зазначалося раніше, кожен образ починається з базового образу. Зазвичай таким базовим образом є один з офіційних образів, доступних у Docker Hub, наприклад дистрибутив Linux - Alpine, Ubuntu або CentOS. Проте також можна створити образ «з нуля». Docker Hub — це публічний реєстр контейнерних образів. Це центральний ресурс, який добре підходить для поширення публічних контейнерних образів. Реєстр доступний за адресою: https://hub.docker.com/

Кожен шар містить лише різницю (delta) змін відносно попереднього набору шарів. Вміст кожного шару відображається у спеціальний каталог на хост-системі, який зазвичай є підкаталогом `/var/lib/docker/`. Оскільки шари є незмінними, їх можна кешувати, і вони ніколи не стають застарілими. Це є великою перевагою.

Коли Docker Engine створює контейнер з образу, він додає поверх цього стеку незмінних шарів записуваний шар контейнера. Після цього стек виглядає так:

![image-20260322203047947](media/image-20260322203047947.png)

рис.9. Записуваний шар контейнера

Шар контейнера позначено як `read/write (r/w)`. Ще однією перевагою незмінності шарів образу є те, що вони можуть спільно використовуватися багатьма контейнерами, створеними з цього образу. Усе, що потрібно для кожного контейнера, — це тонкий записуваний шар контейнера, як показано на наступному рисунку.

![image-20260322203406827](media/image-20260322203406827.png)

рис.10. Кілька контейнерів, що спільно використовують ті самі шари образу

Ця техніка, звичайно, призводить до значного зменшення використання ресурсів. Крім того, це допомагає скоротити час запуску контейнера, оскільки після завантаження шарів образу в пам’ять (що відбувається лише для першого контейнера) потрібно створити лише тонкий шар контейнера.

Docker використовує техніку copy-on-write під час роботи з образами. Copy-on-write — це стратегія спільного використання та копіювання файлів для досягнення максимальної ефективності. Якщо шар використовує файл або каталог, що вже є в одному з нижніх шарів, то він просто використовує його. Якщо ж шар хоче змінити, наприклад, файл із нижнього шару, то спочатку копіює цей файл у цільовий шар, а потім змінює його. На наступному рисунку показано, що це означає:

![image-20260322203646184](media/image-20260322203646184.png)

рис.11. Docker-образ, що використовує copy-on-write

Другий шар хоче змінити файл File 2, який знаходиться в базовому шарі. Тому він спочатку копіює цей файл у свій шар, а потім змінює його. Тепер припустимо, що ми перебуваємо у верхньому шарі на попередньому рисунку. Цей шар використовуватиме File 1 з базового шару, а File 2 і File 3 — з другого шару.
...
- Далі з образу створюється контейнер (див. рис.12. Create). У цей момент Docker готує ізольоване середовище: створює простори імен, налаштовує обмеження ресурсів і додає верхній шар файлової системи. Контейнер уже існує як об’єкт, але ще не виконується.

- Після запуску контейнера (див. рис.12. Start) стартує головний процес. З цього моменту контейнер переходить у робочий стан, використовує процесор, пам’ять, мережу і працює так само, як звичайний сервіс у системі.

- Зупинка контейнера (див. рис.12. Stop) означає завершення його процесів. Контейнер перестає виконуватися, але його стан і файловий шар зберігаються. Це дозволяє пізніше знову запустити контейнер і продовжити роботу з тим самим середовищем.

- Перезапуск контейнера (див.12. Restart) є комбінацією зупинки та повторного запуску. Контейнер отримує новий запуск процесів, але використовує той самий образ і той самий верхній файловий шар.

- Видалення контейнера (див. рис.12. Rm ) є завершальним етапом життєвого циклу. Після цього контейнер і його верхній файловий шар зникають, а пов’язані з ним ресурси звільняються. Образ при цьому залишається у системі і може використовуватися для створення нових контейнерів.
...
![Container lifecycle: create, start, pause, unpause, stop, restart, kill, and remove a container.](media/container-lifecycle-rev2.png)

рис.12. Автомат станів контейнеру (джерело https://labs.iximiuz.com/tutorials/docker-container-management-commands).

На рис.13 показана як працює команда запуску контейнеру  `docker run` . Ця команда не є однією атомарною операцією, натомість складається з кількох кроків: отримання образу, створення контейнера, підключення до мережі, запуск процесів і, за потреби, приєднання до стандартних потоків введення та виведення.
...
рис.13. Як працює Docker Run  ([Джерело](https://labs.iximiuz.com/tutorials/docker-run-vs-attach-vs-exec))
...
![image-20260323150351782](media/image-20260323150351782.png)

рис.14. Збереження власних даних в контейнерах
...
рис.15. Мережа контейнерів ([Джерело](https://iximiuz.com/en/posts/docker-publish-container-ports/))
...
рис.16. Відображення портів ([Джерело](https://manfredmlange.medium.com/port-mapping-with-docker-585897afda3f))
...
1. Що таке контейнер?
2. Яка принципова відмінність між віртуальною машиною та контейнером?
3. Що таке простір імен? Як він використовується в контейнерних технологіях?
4. Який механізм надає можливість обмежити використання ресурсів в контейнерах?
5. Який механізм надає можливість роботи з файлами в контейнерах?
6. todo
