...
- [ ] Для того щоб вийти з режиму перегляду натисніть `Ctrl+C`.
...
Повторно зареєструйтеся як користувач (замість `імя_користувача` вкажіть Вашого користувача):
...
- [ ] Запустіть новий контейнер і “прикріпіть” volume.  Для MariaDB дані всередині контейнера зберігаються у каталозі `/var/lib/mysql`, тому volume монтуємо саме туди:
...
Сервіси `mariadb` та `adminer` мають мати статус `up`.
